...
def print_error(txt): st.error(txt)
def print_warn(txt): st.warning(txt)
def print_ok(txt): st.success(txt)
def print_res(txt): st.info(txt)
...
def heatmap(dfM, outfile, cmap='Greens'):
    ...
    colors = ["white", "lightgray", "steelblue", "blue", "darkgreen"]
...
def Run_tax_myphage(fasta_file, base, prefix, mash_dist, threads, HOME, verbose=True):
    cwd = os.getcwd()
    VMR_path = os.path.join(HOME, 'VMR.xlsx')
    blastdb_path = os.path.join(HOME, 'Bacteriophage_genomes.fasta')
    ICTV_path = os.path.join(HOME, 'ICTV.msh')
    ...
    out_csv_of_taxonomy = prefix+"Output_of_taxonomy.csv"
    ...
    print('mash', 'dist', '-d' , mash_dist,  '-p', threads, ICTV_path, query)
    #run mash to get top hit and read into a pandas dataframe
    mash_output = subprocess.getoutput(f"mash dist -d {mash_dist} -p {threads} {ICTV_path} {query}")
    ...
    im = heatmap(PMV.dfM, heatmap_file)
